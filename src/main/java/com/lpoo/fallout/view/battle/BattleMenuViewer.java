...
public class BattleMenuViewer extends Viewer<BattleMenuModel, LanternaGUI> {
    private List<WordRenderer> optionRendererList;
    ...
    public BattleMenuViewer(@NotNull BattleMenuModel menuModel) {
        super(menuModel);
        optionRendererList = new ArrayList<>();
        Position curPosition = new Position(10, 215);
        for (int i = 0; i < BattleMenuModel.OPTION.values().length; i++) {
            WordRenderer newOptionWord = new WordRenderer(BattleMenuModel.OPTION.valueOfIndex(i).label, curPosition);
            newOptionWord.buildImage();
            optionRendererList.add(newOptionWord);
